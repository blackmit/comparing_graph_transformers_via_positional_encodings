...
from graphgps.loader.split_generator import (prepare_splits,
                                             set_dataset_splits)
from graphgps.loader.bamultishapes_loader import load_bamultishapes_dataset 
from graphgps.loader.brec_loader import load_brec_dataset
CUSTOM_LOADERS = [load_bamultishapes_dataset, load_brec_dataset]
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith('PyG-'):
        ...
        for loader in CUSTOM_LOADERS:
            dataset = loader(format=format, name=name, dataset_dir=dataset_dir)
            if dataset is not None:
                # if a dataset was successfully loaded, use it
                break 
        # if no dataset was loaded, raise an error
        if dataset is None:
            raise ValueError(f"Custom dataset {name} not supported")
...
def one_hot_encoding_edge_features(
    data,
    num_classes : int,
    offset_by_one : bool=False
):
    
    if not hasattr(data, "edge_attr"):
        return data
    offset = 1 if offset_by_one else 0
    data.edge_attr = one_hot(data.edge_attr-offset, num_classes=num_classes)
    return data
...
def preformat_ZINC(dataset_dir, name):
    ...
    # convert the edge features to one hot encodings
    pre_transform_in_memory(
        dataset, 
        partial(one_hot_encoding_edge_features, num_classes=3, offset_by_one=True)
    )
    return dataset
