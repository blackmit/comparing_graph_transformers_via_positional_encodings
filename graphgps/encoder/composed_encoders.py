...
from graphgps.encoder.rpe_transformer_encoder import RPETransformerEncoder
from graphgps.encoder.ign_rpe_encoder import IGNRPEEncoder
from graphgps.encoder.deepsets_ape_encoder import DeepSetsAPEEncoder
...
# Combine both DeepSets and RPETransformer positional encodings.
for ds_enc_name, ds_enc_cls in ds_encs.items():
    register_node_encoder(
        f"{ds_enc_name}+DeepSetsAPEEncoder+RPETransformerBias",
        concat_node_encoders([ds_enc_cls, DeepSetsAPEEncoder, RPETransformerEncoder],
                             ['RPE', 'RPE'])
    )

# Special case for different RPE encoders
for ds_enc_name, ds_enc_cls in ds_encs.items():
    register_node_encoder(
        f"{ds_enc_name}+RPETransformerBias+IGNRPEEncoder",
        concat_node_encoders([ds_enc_cls, RPETransformerEncoder, IGNRPEEncoder],
                             ['RPE', 'RPE'])
    )
    register_node_encoder(
        f"{ds_enc_name}+RPETransformerBias",
        concat_node_encoders([ds_enc_cls, RPETransformerEncoder],
                             ['RPE'])
    )


register_node_encoder(
        f"RPETransformerBias+IGNRPEEncoder",
        concat_node_encoders([RPETransformerEncoder, IGNRPEEncoder],
                             ['RPE'])
    )

register_node_encoder(
        f"DeepSetsAPEEncoder+RPETransformerBias",
        concat_node_encoders([DeepSetsAPEEncoder, RPETransformerEncoder],
                             ["RPE"])
    )
